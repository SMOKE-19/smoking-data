...
def restore_dataset_to_dataset(
    ...
    if max_workers <= 1:
        ...
        with concurrent.futures.ProcessPoolExecutor(
            ...
            results = list(
                executor.map(_restore_parquet_to_parquet_worker, worker_args)
            )
...
def restore_dataset_to_dataset_profiled(
    ...
    if max_workers <= 1:
        results = [
            _restore_parquet_to_parquet_profiled_worker(args) for args in worker_args
        ]
    else:
        with concurrent.futures.ProcessPoolExecutor(
            max_workers=max_workers
            ...
            results = list(
                executor.map(_restore_parquet_to_parquet_profiled_worker, worker_args)
            )
